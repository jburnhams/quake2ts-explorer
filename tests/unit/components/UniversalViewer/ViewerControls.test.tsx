...
describe('ViewerControls', () => {
  ...
  describe('Video Recording Controls', () => {
    ...
    it('displays estimated size when recording', () => {
        render(<ViewerControls {...defaultProps} onScreenshot={jest.fn()} onStartRecording={jest.fn()} onStopRecording={jest.fn()} isRecording={true} recordingTime={10} recordingSizeEstimate={1048576} />);
        expect(screen.getByText('1.0 MB')).toBeInTheDocument();
    });

    it('highlights time in yellow if warning limit exceeded', () => {
        render(<ViewerControls {...defaultProps} onScreenshot={jest.fn()} onStartRecording={jest.fn()} onStopRecording={jest.fn()} isRecording={true} recordingTime={61} />);
        const timer = screen.getByText('1:01');
        expect(timer).toHaveStyle('color: yellow');
    });

    it('shows normal time color if below warning limit', () => {
        render(<ViewerControls {...defaultProps} onScreenshot={jest.fn()} onStartRecording={jest.fn()} onStopRecording={jest.fn()} isRecording={true} recordingTime={59} />);
