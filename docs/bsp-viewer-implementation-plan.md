...
This document outlines a technical plan for extending the BSP viewer with interactive features. It has been updated to leverage new APIs available in `quake2ts@0.0.177`.

## 1. Core Concepts

New, higher-level APIs in `quake2ts` simplify the implementation of the desired features. The application logic can now focus more on UI and state management.

-   **Picking:** We will use the `map.pickEntity(ray)` method to directly identify which entity is under the user's cursor. This is **now available** in `quake2ts`.
-   **Metadata:** We will use the `map.entities.getUniqueClassnames()` method to easily populate our UI legend. This is **now available** in `quake2ts`.
-   **Visibility:** Controlling entity visibility still requires a manual implementation in the application's render loop, as a high-level API for this is not yet available.
...
### Task 2: Implement Toggling Visibility (Medium)
...
2.  **Modify `BspAdapter.ts`**:
    ...
    -   In the `render` method, before drawing each model, retrieve its corresponding entity (by `modelIndex - 1`).
    -   If the entity's `classname` is in the `hidden` set, skip the `gl.drawElements` call for that model's surface.
3.  **Update `UniversalViewer.tsx`**:
    -   When the `hiddenClassnames` set changes, call the adapter's `setHiddenClasses` method to trigger a re-render.
...
## 3. Future Library Enhancements

The current `quake2ts` API is sufficient for all the features outlined above. However, a high-level API for visibility filtering would further simplify the application code.

-   **Proposed Feature:** Extend `createBspSurfaces` or `buildBspGeometry` to accept an optional `hiddenClassnames: Set<string>` parameter. The library would then handle the filtering internally, which is likely to be more performant than the application-level filtering proposed in Task 2.
