# Technical Overview: BSP Viewer Enhancements (V2)

This document outlines a technical plan for extending the BSP viewer with interactive features. It assumes that the `quake2ts` library has been updated with the features proposed in `docs/quake2ts-feature-requests.md`.

## 1. Core Concepts

With the proposed enhancements to `quake2ts`, the application logic becomes much simpler. The library will now handle the complex tasks of ray-casting and geometry filtering, allowing the application to focus on UI and state management.

-   **Picking:** We will use the new `map.pickEntity(ray)` method to directly identify which entity is under the user's cursor.
-   **Visibility:** We will use the new `buildBspGeometry(..., { hiddenClassnames })` option to efficiently show and hide groups of entities.
-   **Metadata:** We will use the new `map.entities.getUniqueClassnames()` method to easily populate our UI legend.
...
1.  **Modify `BspAdapter.ts`**:
    -   Add a public method `getUniqueClassnames(): string[]` that simply returns `this.map?.entities.getUniqueClassnames() ?? []`.
...
2.  **Modify `BspAdapter.ts`**:
    ...
    -   Inside this method, store the set and re-build the renderable geometry by calling `buildBspGeometry` again, passing the new set to the `hiddenClassnames` option.
    -   This will trigger a re-render with the correct objects hidden.
3.  **Update `UniversalViewer.tsx`**:
    -   When the `hiddenClassnames` set changes, call the adapter's `setHiddenClasses` method.
...
1.  **Create a Ray-Casting Utility (`src/utils/camera.ts`)**:
    -   Implement a function `createPickingRay(camera: Camera, mouseCoords: { x: number, y: number }, viewport: { width: number, height: number }): Ray`. This is still needed to get a ray from the mouse coordinates, but the intersection logic is no longer the application's responsibility.
2.  **Modify `UniversalViewer.tsx`**:
